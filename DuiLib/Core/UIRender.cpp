...
TDRAWINFO_UI::TDRAWINFO_UI(LPCTSTR lpsz)
{
    Clear();
    sDrawString = lpsz;
}
...
void TDRAWINFO_UI::Clear()
{
    sDrawString.Empty();
    sImageName.Empty();
    ::ZeroMemory(&bLoaded, sizeof(TDRAWINFO_UI) - offsetof(TDRAWINFO_UI, bLoaded));
    uFade = 255;
}
            ...
            case _T('f'):  // Font
                {
                    ...
                    if( pstrTemp != pstrText ) {
                        TFONTINFO_UI* pFontInfo = pManager->GetFontInfo(iFont);
                        aFontArray.Add(pFontInfo);
                        pTm = &pFontInfo->tm;
                        ::SelectObject(hDC, pFontInfo->hFont);
                    }
                    ...
                    if( sName.IsEmpty() ) { // Italic
						...
						if( pImageInfo ) {
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                pstrNextStart = NULL;
                                if( bDraw && bLineDraw ) {
                                    CRectUI rcImage(pt.x + cxOffset, pt.y + cyLineHeight - iHeight, pt.x + + cxOffset + iWidth, pt.y + cyLineHeight);
									iVAlign = DT_BOTTOM;
