...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    CManagerUI::~CManagerUI()
    {
        ...
        for (int i = 0; i < m_aAsyncNotify.GetSize(); i++) delete static_cast<struct TNOTIFY_UI*>(m_aAsyncNotify[i]);
    ...
    void CManagerUI::ReapObjects(CControlUI* pControl)
    {
        ...
        for( int i = 0; i < m_aAsyncNotify.GetSize(); i++ ) {
            struct TNOTIFY_UI* pMsg = static_cast<struct TNOTIFY_UI*>(m_aAsyncNotify[i]);
    ...
    BOOL CManagerUI::SetTimer(CControlUI* pControl, UINT nTimerID, UINT uElapse)
    {
        ...
        struct TIMERINFO_UI* pTimer = new struct TIMERINFO_UI;
    ...
    BOOL CManagerUI::SetNextTabControl(BOOL bForward)
    {
        ...
        if( pControl == NULL ) {  
            if( bForward ) {
                // Wrap around
                struct TFINDTABINFO_UI info2 = { 0 };
    ...
    void CManagerUI::SendNotify(struct TNOTIFY_UI& Msg, BOOL bAsync /*= FALSE*/, BOOL bEnableRepeat /*= TRUE*/)
    {
        ...
        if( !bAsync ) {
		    ...
		    struct TNOTIFY_UI *pMsg = new struct TNOTIFY_UI;
            if (m_bUsedVirtualWnd) pMsg->sVirtualWnd = Msg.sVirtualWnd;
    ...
    void CManagerUI::RemoveAllWindowCustomAttribute()
    {
	    ...
	    for( int i = 0; i< m_mWindowAttrHash.GetSize(); i++ ) {
		    if(LPCTSTR key = m_mWindowAttrHash.GetAt(i)) {
			    pCostomAttr = static_cast<CStringUI*>(m_mWindowAttrHash.Find(key));
			    delete pCostomAttr;
		    }
	    }
    ...
    CStringUI CManagerUI::GetWindowXML()
    {
        ...
        //for( int i = 0; i< m_ResInfo.m_CustomFonts.GetSize(); i++ ) {
        //    if(LPCTSTR key = m_ResInfo.m_CustomFonts.GetAt(i)) {
        //        sDefaultAttr = static_cast<CStringUI*>(m_ResInfo.m_CustomFonts.Find(key))->GetData();
        ...
        for( int i = 0; i< m_ResInfo.m_AttrHash.GetSize(); i++ ) {
            if(LPCTSTR key = m_ResInfo.m_AttrHash.GetAt(i)) {
                sDefaultAttr = static_cast<CStringUI*>(m_ResInfo.m_AttrHash.Find(key))->GetData();
                sDefaultAttr.Replace(_T("\""), _T("&quot;"));
                sWindowXML.Append(_T("\n\t<Default name=\" "));
                sWindowXML.Append(key);
                sWindowXML.Append(_T("\" value=\" "));
    ...
    void CManagerUI::AddMultiLanguageString(int id, LPCTSTR pStrMultiLanguage)
    {
	    ...
	    CStringUI* pMultiLanguage = new CStringUI(pStrMultiLanguage);
	    if (pMultiLanguage != NULL)
	    {
		    CStringUI* pOldMultiLanguage = static_cast<CStringUI*>(m_SharedResInfo.m_MultiLanguageHash.Set(idBuffer, (LPVOID)pMultiLanguage));
    ...
    void CManagerUI::RemoveAllMultiLanguageString()
    {
	    ...
	    for( int i = 0; i< m_SharedResInfo.m_MultiLanguageHash.GetSize(); i++ ) {
		    if(LPCTSTR key = m_SharedResInfo.m_MultiLanguageHash.GetAt(i)) {
			    pMultiLanguage = static_cast<CStringUI*>(m_SharedResInfo.m_MultiLanguageHash.Find(key));
