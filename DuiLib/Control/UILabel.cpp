    ...
    if (m_bNeedEstimateSize) {
        ...
        if ((m_uTextStyle & DT_SINGLELINE) != 0) {
            ...
            if (m_cxyFixedLast.cx == 0) {
                ...
                if (m_bShowHtml) {
                    int nLinks = 0;
                    CRenderUI::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, NULL, NULL, nLinks, m_iFont, DT_CALCRECT | m_uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                } else {
                    CRenderUI::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, m_iFont, DT_CALCRECT | m_uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }
...
void CLabelUI::PaintText(HDC hDC) {
    ...
    if (!GetEnabledEffect()) {
        ...
        if (IsEnabled()) {
            if (m_bShowHtml) {
                CRenderUI::DrawHtmlText(hDC, m_pManager, rc, m_sText, m_dwTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
            } else {
                CRenderUI::DrawText(hDC, m_pManager, rc, m_sText, m_dwTextColor, m_iFont, m_uTextStyle);
            }
        } else {
            if (m_bShowHtml) {
                CRenderUI::DrawHtmlText(hDC, m_pManager, rc, m_sText, m_dwDisabledTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
            } else {
                CRenderUI::DrawText(hDC, m_pManager, rc, m_sText, m_dwDisabledTextColor, m_iFont, m_uTextStyle);
            }
        }
    } else {
#ifdef _USE_GDIPLUS
